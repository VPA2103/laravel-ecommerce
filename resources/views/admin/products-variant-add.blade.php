...
<div class="main-content-inner">
    <div class="main-content-wrap">
        ...
        <!-- new-category -->
         <div class="wg-box">
            <form class="form-new-product form-style-1" action="{{ route('admin.products-variant.store') }}" method="POST"
                ...
                <fieldset class="name">
                    ...
                    <select name="color" class="form-select" required>
                        ...
                        @foreach($colors as $color)
                            <option value="{{ strtolower($color) }}"
                                {{ (isset($products_variant) && strtolower($products_variant->color ?? '') === strtolower($color)) ? 'selected' : '' }}>
                                {{ $color }}
                            </option>
                        @endforeach
                    </select>
                ...
                <fieldset class="name">
                    ...
                    <input class="flex-grow" type="number" step="0.01" placeholder="Product variant price"
                        name="price" value="{{ $products_variant->price ?? '' }}" required>
                </fieldset>
                @error('price')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror
                <fieldset class="name">
                    ...
                    <input class="flex-grow" type="number" placeholder="Product variant stock"
                        name="stock" value="{{ $products_variant->stock ?? '' }}" required>
                </fieldset>
                @error('stock')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror
